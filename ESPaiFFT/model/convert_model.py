...
def load_pytorch_model():
    ...
    # Utwórz architekturę ResNet18
    model = models.resnet18(weights=None)
    
    # Dostosuj dla single channel input
    model.conv1 = torch.nn.Conv2d(1, 64, kernel_size=7, stride=2, padding=3, bias=False)
    
    # Dostosuj output dla 2 klas
    model.fc = torch.nn.Linear(512, 2)
    
    # Mapuj "shortcut" → "downsample" jeśli konieczne
    new_state_dict = {}
    for key, value in state_dict.items():
        if 'shortcut' in key:
            # Zamień shortcut na downsample
...
def pytorch_to_tflite_direct():
    ...
    # Layer1 - 2x residual block (64 channels, stride 1)
    x = residual_block(x, 64, stride=1)
    x = residual_block(x, 64, stride=1)
    
    # Layer2 - 2x residual block (128 channels, stride 2)
    x = residual_block(x, 128, stride=2)
    x = residual_block(x, 128, stride=1)
    
    # Layer3 - 2x residual block (256 channels, stride 2)
    x = residual_block(x, 256, stride=2)
    x = residual_block(x, 256, stride=1)
    
    # Layer4 - 2x residual block (512 channels, stride 2)
...
#define DRONE_MODEL_SEGMENT_SIZE {SEGMENT_SIZE}
...
"""
    ...
    # Generuj tablicę wskaźników na segmenty
    c_code += f"""
