...
export const MapComponent = () => {
  ...
  useEffect(() => {
    ...
    espNodes.forEach((node) => {
      ...
      circle.bindPopup(`${t.map.sensor}: ${node.id}<br/>${t.map.route}: ${node.lineId}`);
    ...
    dronePositions.forEach((position, index) => {
      ...
      droneMarker.bindPopup(`🚁 ${t.map.drone} #${index + 1}<br/>${t.map.detection}: 200m`);
        ...
        <div className="relative">
          ...
          {showSearchResults && searchResults.length === 0 && searchQuery.length >= 2 && (
            <div className="absolute top-full mt-2 w-full bg-card border border-border rounded-lg shadow-lg p-3 z-50">
              <p className="text-xs text-muted-foreground text-center">{t.map.noResults}</p>
            </div>
          )}
        </div>
      </div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-3">
                ...
                <div className="flex-1 pt-1">
                  <div className="font-medium text-sm">{t.map.selectStart}</div>
                  {missionStartPoint ? (
                    <p className="text-xs text-green-600 dark:text-green-400">
                      {t.deploymentDialog.startPoint}: {missionStartPoint[0].toFixed(4)}°, {missionStartPoint[1].toFixed(4)}°
                    </p>
                  ) : selectionStep === 'start' ? (
                    ...
                    <p className="text-xs text-muted-foreground">{t.map.selectStart}</p>
                  )}
                </div>
              </div>
              ...
              <div className="flex gap-3">
                ...
                <div className="flex-1 pt-1">
                  <div className="font-medium text-sm">{t.map.selectEnd}</div>
                  {missionEndPoint ? (
                    <p className="text-xs text-red-600 dark:text-red-400">
                      {t.deploymentDialog.endPoint}: {missionEndPoint[0].toFixed(4)}°, {missionEndPoint[1].toFixed(4)}°
                    </p>
                  ) : selectionStep === 'end' ? (
                    <p className="text-xs text-blue-600 dark:text-blue-400 animate-pulse">
                      {t.deploymentDialog.clickMap}
                    ...
                    <p className="text-xs text-muted-foreground">{t.map.selectEnd}</p>
                  ) : (
                    <p className="text-xs text-gray-400">{t.map.selectStart}</p>
                  )}
                </div>
              </div>
            </div>
        ...
        <div className="absolute bottom-4 left-4 z-40">
          <div className="bg-card border border-border rounded-lg shadow-lg p-3 space-y-2 w-72">
            <div className="flex items-center justify-between">
              <h4 className="font-semibold text-sm">{t.map.plannedRoute}</h4>
              <button
                onClick={() => {
                  setMissionStartPoint(null);
                  setMissionEndPoint(null);
            ...
            {missionStartPoint && (
              <div className="flex items-center gap-2 text-xs">
                ...
                <div>
                  <div className="font-medium">{t.map.start}</div>
            ...
            {missionEndPoint && (
              <div className="flex items-center gap-2 text-xs">
                ...
                <div>
                  <div className="font-medium">{t.map.end}</div>
                  <div className="text-muted-foreground">
            ...
            {missionStartPoint && missionEndPoint && (
              <div className="pt-2 border-t border-border text-xs">
                <div className="text-muted-foreground">{t.map.distance}:</div>
